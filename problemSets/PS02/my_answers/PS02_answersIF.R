...
barplot(cbind(expected, observed ), legend.text = rows, 
        names.arg = c("ns", "sgw", "br", "ns", "sgw", "br"),
...
cat(str_glue("We {txt}reject the null hypothesis that the two sets are from the same population"))
cat(str_glue("{cells_under} observed cell(s) have less than 5 values"))
