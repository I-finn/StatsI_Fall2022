...
# 2 diff means, medians - male height skewed left
# Formulate our null hypothesis:
# What is our null hypothesis? no difference, ie same distribution
# Where do we set alpha? (i.e. rejecting null hypothesis when we shouldn't; 5%
# note that we don't get to decide beta - i.e. the probability of incorrectly 
# accepting the null hypothesis)
# Is a one-tail or two-tail test more appropriate?  two tail - difference, not 1 >,< other
...
var.test(height ~ sex_f,
         ...
         conf.level = 0.95)
# run diff test i same/not var - see param in t.test
...
t.test(height ~ sex_f, # Use formula to group
       ...
       var.equal = FALSE, # The default is FALSE
       alternative = "two.sided", 
       conf.level = .95) # Try changing this critical value - R splits alpa in 2
