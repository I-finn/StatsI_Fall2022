...
iris %>%
  ...
  # data is the internal reference generated by nest_by on list ; alt = .
...
iris %>%
  filter(Sepal.Length > 4.5) %>%   # filter on rows
  group_by(Species) %>%            # group by columns
  mutate(mean_x = mean(Sepal.Length),
         mean_y = mean(Sepal.Width)) %>%   # adds 2 extra columns
  ungroup() %>%
  ggplot(aes(Sepal.Length, Sepal.Width)) +
  geom_point(aes(colour = Species)) +
  geom_point(aes(x = mean_x, y = mean_y, colour = Species),
...
dat1 <- read_csv("movies.csv", show_col_types = FALSE)   # if don't want col info to show
class(dat1) # the movies_csv() function creates a tibble

spec(dat1) # list of columns & their types

dat   # give different outputs
dat1  # ie diff views of the data
...
mutate(dat, rel_mon = month.abb[thtr_rel_month])   # creates a new column @end
# month.abb - "Jan", "Feb", ...

# if you wanted to change original column - transmute
...
# to find out which actor was most common
dat %>%
  select(actor1) %>% # select one column
  group_by(actor1) %>% # group data by month
  summarise(n = n()) %>% # perform a summary operation (count the n per month)
...
dat %>%  
  ...
  group_by(month) %>% 
  summarise(n = n()) %>%
  mutate(prop_month = round(n / sum(n), 3), count_month = n) %>% # mutate after our summarise to find the proportion
  arrange(desc(prop_month))

dat %>%  
  select(thtr_rel_month, genre) %>%
  mutate(month = month.abb[thtr_rel_month]) %>% 
  ...
  group_by(month) %>% 
  summarise(n = n()) %>%
  mutate(prop_month = round(n / sum(n), 3), count_month = n, prop_tot = round(n / tot_n, 2) ) %>% # mutate after our summarise to find the proportion
  arrange(desc(prop_month))
...
dat1 %>% 
  ...
  filter(runtime > 120) %>%
  select(title, runtime, thtr_rel_year, best_pic_win) %>%
...
dat1 %>% 
  summarise(mean_runtime = mean(runtime, na.rm = TRUE), n= n(), sd_runtime = sd(runtime, na.rm = TRUE))
...
# 1 2 means   106 ; 143
# 2 sample sizes  7 ; 650
# 3 SDs of 2 pops  19.4 ; 43.7


t.test(x = best_runtime, mu = 106, alternative = "greater", var.equal = FALSE)  # ok

t.test(x= all_runtime, mu = 143, alternative = "less", var.equal = FALSE)       # doesn't work
